...
def train(cfg: DictConfig):
    ...
    # Load an existent policy 
    policy.load_state_dict(torch.load("/home/adame/torchAirBender/outputs/policies/TT/trajectory_tracking_final.pt", map_location=device))
    ...
    gates_position, gates_rpy = load_gates_from_yaml(
        "/home/adame/torchAirBender/miscellaneous/race_tracks/uzh_7g.yaml"
    )

    renderer = RacingRenderer(
        gates_position=gates_position.cpu().numpy(),   # (N, 3) ENU
        gates_rpy=gates_rpy.cpu().numpy(),             # (N, 3) or (3,) degrees
        gate_mesh_path="/home/adame/torchAirBender/miscellaneous/gate.obj",
        gate_scale=1.0,
        gate_color=(0.1, 0.6, 1.0),
        ref_trajectory=None,                           # or (T, 3) numpy array
        trajectory=traj_env0[:, :17].cpu().numpy(),
...
def test(cfg: DictConfig):
    ...
    renderer = RacingRenderer(
        gates_position=gates_position,   # (N, 3) ENU
        gates_rpy=gates_rpy,             # (N, 3) or (3,) degrees
        gate_mesh_path="/home/adame/torchAirBender/miscellaneous/gate.obj",
        gate_scale=1.0,
        gate_color=(0.5, 0.0, 0.5),
        ref_trajectory=traj[:, 17:20].cpu().numpy(),                           # or (T, 3) numpy array
